...
interface Props { status: 'green' | 'yellow' | 'red'; value?: number; showLabel?: boolean; }

export function MarginBadge({ status, value, showLabel = true }: Props) {
  const { t } = useTranslation();
  const colors = { green: 'bg-emerald-100 text-emerald-800 border-emerald-200', yellow: 'bg-amber-100 text-amber-800 border-amber-200', red: 'bg-red-100 text-red-800 border-red-200' };
