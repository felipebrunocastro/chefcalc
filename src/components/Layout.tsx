...
import { LayoutDashboard, Package, ChefHat, Calculator, FileDown, Menu, X } from 'lucide-react';
...
export function Layout({ current, onNavigate, children }: Props) {
  ...
  const navigate = (view: AppView) => { onNavigate(view); setMobileMenuOpen(false); };

  return (
    <div className="min-h-screen flex flex-col">
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 flex items-center h-14 gap-4">
          <button className="md:hidden p-1.5 text-gray-500 hover:text-gray-700 rounded-md" onClick={() => setMobileMenuOpen(m => !m)}>
            ...
            {NAV_ITEMS.map(({ view, icon: Icon, key }) => (
              <button key={view} onClick={() => navigate(view)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                  current === view ? 'bg-amber-50 text-amber-600' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-100'
                }`}>
                <Icon size={14} />{t(key)}
              </button>
            ))}
            ...
            {NAV_ITEMS.map(({ view, icon: Icon, key }) => (
              <button key={view} onClick={() => navigate(view)}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors mb-0.5 ${
                  current === view ? 'bg-amber-50 text-amber-600' : 'text-gray-600 hover:bg-gray-100'
                }`}>
                <Icon size={16} />{t(key)}
              </button>
            ))}
